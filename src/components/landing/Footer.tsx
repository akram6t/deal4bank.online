...
export default function Footer({ settings }: FooterProps) {
  ...
  const tagline = settings?.tagline || 'Your trusted financial partner';
  const phone = settings?.phone || '9243956990';
  const email = settings?.email || 'info@deal4bank.online';
  const whatsapp = settings?.whatsapp || '9243956990';
  const copyright = settings?.copyright || '© 2024 deal4bank.com. All rights reserved.';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 relative">
        <div className="flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <div className="text-center md:text-left">
            <h3 className="text-lg font-bold text-gray-50 dark:text-blue-400 mb-2">{name}</h3>
            <p className="text-gray-50 dark:text-gray-300 text-sm">{tagline}</p>
          </div>
          ...
          <div className="flex flex-wrap justify-center gap-6 text-sm text-gray-50 dark:text-gray-300">
            <div className="flex items-center">
              ...
              <a href={`https://wa.me/91${whatsapp}`} target='_blank'>{phone}</a>
            </div>
            <div className="flex items-center">
